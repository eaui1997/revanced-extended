...
function get_patches_key() {
    ...
    export excluded_string=$(tail -n +$excluded_start patches/$patch_file | head -n "$(( included_start - excluded_start ))"  | grep '^[^#[:blank:]]')
    export included_string=$(tail -n +$included_start patches/$patch_file | grep '^[^#[:blank:]]')
